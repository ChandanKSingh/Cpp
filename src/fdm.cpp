...
FDMBase::FDMBase(double _x_dom, unsigned long _J,
                 ...
                 ConvectionDiffusionPDE* _pde): x_dom(_x_dom),
                 J(_J), t_dom(_t_dom), N(_N), pde(_pde) {}
...
void FDMEulerExplicit::calculate_inner_domain() {
    // Only use inner result indicies (1 to J-2)
    for (unsigned long j=1; j<J-1; j++) {
        ...
        // Diffrencing coefficients (see \alpha, \beta and \gamma in text)
        alpha = dt_sig - dt_sig_2;
        beta = dx * dx - (2.0 * dt_sig) + (dt * dx * dx * (pde->zero_coeff(prev_t, x_values[j])));
        gamma = dt_sig + dt_sig_2;

        // Update inner value of spatial discretisation grid (Explicit Euler)
